...
Matrix *tifImagetoMatrix(std::string imageName) {
...
//                    image->setValues(y,x,1);
//                else
//                    image->setValues(y,x,0);
                double z = TIFFGetB(TiffPixel) + TIFFGetG(TiffPixel) + TIFFGetR(TiffPixel);
//                vectorLine.push_back((z/3.0)/255.0);
                image->setValues(y,x,(z/3.0)/255.0);
...
//            double temp =0;
//            for (int w = 0; w < image->getWidth()/2; ++w) {
//                temp = image->getValues(h,first);
//                image->setValues(h,first,image->getValues(h,last));
//                image->setValues(h,last,temp);
//                first++;
//                last--;
//            }
//        }
...
//    std::cout << h->getValues()[0][0] << std::endl;
    ...
    Matrix *image = tifImagetoMatrix("00001.tif");

    for (int h = 0; h < image->getHeight(); ++h) {
        for (int w = 0; w < image->getWidth(); ++w) {
            std::cout << image->getValues(h,w);
        }
        std::cout << std::endl;
    }
